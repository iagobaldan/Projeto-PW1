...
export class Sitting extends State {
    constructor(game) { super('SITTING', game); }
    enter() {
    ...
    handleInput(input) {
        if (['ArrowLeft','ArrowRight'].some(k => input.includes(k))) this.game.player.setState(states.RUNNING, 1);
        else if (input.includes('Enter')) this.game.player.setState(states.ROLLING, 2);
    }
}

export class Running extends State {
    constructor(game) { super('RUNNING', game); }
    enter() {
        this.game.player.frameX = 0;
...
export class Jumping extends State {
    constructor(game) { super('JUMPING', game); }
    enter() {
...
export class Falling extends State {
    constructor(game) { super('FALLING', game); }
    enter() {
        this.game.player.frameX = 0;
...
export class Rolling extends State {
    constructor(game) { super('ROLLING', game); }
    enter() {
        this.game.player.frameX = 0;
...
export class Diving extends State {
    constructor(game) { super('DIVING', game); }
    enter() {
        this.game.player.frameY = 6;
        this.game.player.maxFrame = 6;
        this.game.player.frameX = 0;
...
export class HIT extends State {
    constructor(game) { super('HIT', game); }
    enter() {
        this.game.player.frameX = 0;
